...
record FlatCwF : Set where
  field C         : Cat
        ...
        _<<_      : ∀{Γ Δ σ}(f : Hom C Γ Δ) → 
                    Tm Γ (σ [ f ]⁺) → Hom C Γ (Δ < σ)
        Cons-L    : ∀{Γ Δ σ}{f : Hom C Γ Δ}{M : Tm Γ (σ [ f ]⁺)} → 
                    comp C (p σ) (f << M) ≅ f
        Cons-R    : ∀{Γ Δ σ}{f : Hom C Γ Δ}{M : Tm Γ (σ [ f ]⁺)} → 
                    v [ f << M ] ≅ M
        Cons-Nat  : ∀{B Γ Δ σ}{f : Hom C Γ Δ}{g : Hom C B Γ}
                    {M : Tm Γ (σ [ f ]⁺)} → 
                    comp C (f << M) g 
                    ≅ 
                    comp C f g << subst' (Tm B) (sym' Ty-Comp) (M [ g ])
        Cons-Id   : ∀{Δ σ} → (p σ) << v ≅ iden C {Δ < σ}

  -- weakening from Section 3.1.3
  q : ∀{B Γ}(f : Hom C B Γ)(σ : Ty Γ) → 
      Hom C (B < (σ [ f ]⁺)) (Γ < σ) 
